...
function sarbacaneAddField() {
	...
	if ( sarbacaneFieldNumber < 10 ) {
		jQuery( "#sarbacane_desktop_additional_fields" ).append(
			jQuery( "#fieldPattern" ).clone().html().replace( /(FIELDNUMBER)/gi, sarbacaneFieldNumber ).replace( /(PLUSONEFIELD)/g, fieldNumberPlusOne )
		);
	}
...
function sarbacaneSubmitForm() {
	...
	for ( var i = 0; i < sarbacaneFieldNumber; i++ ) {
		...
		if ( (labelField == null || labelField == "" || ( !isEmail && labelField.toLowerCase() == "email" ) ) && ( placeholderField == null || placeholderField == "" ) ) {
